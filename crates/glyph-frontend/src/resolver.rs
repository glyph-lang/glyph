...
use glyph_core::{
    ast::{BinaryOp, ConstDef, EnumDef, Expr, Ident, Item, Literal, Module, StructDef, TypeExpr},
    diag::Diagnostic,
    span::Span,
...
fn resolve_const_reference(
    ...
    if let Some(ResolvedSymbol::Const(target_module, const_name)) = ctx.resolve_symbol(name) {
...
fn find_const_def<'a>(module: &'a Module, name: &str) -> Option<&'a ConstDef> {
    module.items.iter().find_map(|item| match item {
        Item::Const(def) if def.name.0 == name => Some(def),
        _ => None,
    })
}
...
fn find_enum_def<'a>(module: &'a Module, enum_name: &str) -> Option<&'a EnumDef> {
    module.items.iter().find_map(|item| match item {
